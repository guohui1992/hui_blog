...
![手写代码面试题.png](https://cdn.nlark.com/yuque/0/2021/png/1500604/1621676946378-71d6b405-ef4e-42e4-9e42-f9c9aafcefb6.png#averageHue=%23fbf8f6&clientId=ubdaa2908-0c83-4&errorMessage=unknown%20error&from=ui&height=1486&id=u45da4c1b&originHeight=2754&originWidth=2780&originalType=binary&ratio=1&rotation=0&showTitle=false&size=792413&status=error&style=shadow&taskId=ub107fa80-9302-43d4-9f26-7bdbd05341d&title=&width=1500)
## 一、JavaScript 基础
### 1. 手写 Object.create 
思路：将传入的对象作为原型
```javascript
...
```
### 21. 异步任务调度器
实现异步任务调度器，最多允许2个任务同时执行
```javascript
class AnsycScheduler{
	constructor(max){
    // 最大可并发数
    this.max = max;
    // 当前并发任务数
...
// 同时进行的任务最多两个
const scheduler = new AnsyncScheduler(2);
