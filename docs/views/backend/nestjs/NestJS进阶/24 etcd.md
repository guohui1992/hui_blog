...
分布式键值存储：etcd 提供一个简单的键值对存储接口，适合用于服务注册、配置存储等场景。
高可用性：通过 Raft 算法等机制，etcd 提供了强一致性和高可用性。
服务发现与配置管理：etcd 可以用来实现分布式系统中的服务发现，微服务可以通过 etcd 注册自己，并通过它发现其他服务的位置。
2.etcd 的核心概念
键值对存储（Key-Value Store）：etcd 的核心是简单的键值存储，数据以键值对的形式存储。键是唯一的，可以通过键来检索、更新、删除相关的值。

租约（Lease）：租约是 etcd 中的一个核心机制，它为某些键值对提供生命周期管理。租约到期后，关联的键值对将自动失效。这通常用于服务发现中服务注册的有效期管理。

Watch：etcd 提供了 Watch 机制，可以实时监听键的变化，适用于配置变更通知或服务状态变化。
