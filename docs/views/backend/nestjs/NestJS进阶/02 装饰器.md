...
![原型链](image.png)
这张图展示了 JavaScript 中函数和原型对象之间的关系，揭示了它们在原型链上的互相连接。通过这个图来理解 JavaScript 中的对象和函数是如何通过原型链连接在一起的
**主要部分**

1. Functions:

- 表示函数对象，图中展示了Foo、Object 和 Function对象
...
1. 构造函数：constructor 是一个特殊的方法，用于在创建对象实例时初始化对象。
1. 成员变量：类的属性，可以是实例属性或静态属性。
1. 方法：类的行为，可以是实例方法或静态方法。
...
1. 方法装饰器（Method Decorators）：应用于方法，可以用于修改方法的行为。

1. 访问器装饰器（Accessor Decorators）：应用于类的访问器属性（getter 或 setter）。

1. 属性装饰器（Property Decorators）：应用于类的属性。

1. 参数装饰器（Parameter Decorators）：应用于方法参数。
...
1. propertyKey：装饰的成员名称。
1. descriptor：成员的属性描述符。
...
1. target：装饰的目标对象，对于静态成员来说是类的构造函数，对于实例成员是类的原型对象。
1. propertyKey：参数所属的方法的名称。
1. parameterIndex：参数在参数列表中的索引。
