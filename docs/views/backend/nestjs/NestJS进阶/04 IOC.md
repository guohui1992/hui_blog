...
1.1 使用stage3装饰器
proposal-decorators
typescript-compiling#_using-newer-typescript-versions
...
export {}
1.8 类自动访问器装饰器
类自动访问器是一种新的类元素类型，它在类字段前添加 accessor 关键字。自动访问器自动为字段创建 getter 和 setter 方法，并默认将值存储在一个私有槽中。
...
额外初始化逻辑：在类或类成员被完全定义后运行一些额外的初始化代码。
动态行为注入：为类实例或类本身注入一些动态行为或属性。
1.9.1 类装饰器
...
代码补全：在输入代码时，IntelliSense 会根据上下文提供自动补全建议，包括变量、函数、类、模块等。
参数信息：在调用函数时，IntelliSense 会显示该函数的参数信息，包括参数名称和类型。
快速信息：悬停在代码元素上时，IntelliSense 会显示相关信息，包括类型、文档注释等。
错误检查：IntelliSense 会实时检查代码中的语法和类型错误，并提供相应的错误信息和修复建议。
2.tsserver
tsserver 是 TypeScript 的一个服务器，它为编辑器提供 TypeScript 和 JavaScript 语言服务。它通过与编辑器通信，提供语法分析、类型检查、代码补全等功能。tsserver 的主要职责包括：

语法和类型分析：实时分析代码的语法和类型，提供错误检查和代码提示。
项目管理：管理 TypeScript 项目，包括编译选项、模块解析等。
与编辑器集成：通过与编辑器（如 VSCode）通信，提供代码补全、参数提示、快速信息等功能。
3.typescript.tsdk
...
用户设置：在用户设置中指定 typescript.tsdk 时，VSCode 将使用指定路径下的 TypeScript 版本替换内置的 TypeScript 版本。用户设置适用于所有工作区。
工作区设置：在工作区设置中指定 typescript.tsdk 时，可以使用 VSCode 的 TypeScript: Select TypeScript version 命令在当前工作区内切换到指定路径下的 TypeScript 版本。工作区设置仅对当前工作区有效。
配置示例：
...
4.6 示例：类字段
阶段 0：有人建议类中的私有字段可以提高封装性。
阶段 1：一个 TC39 成员支持这个想法，并写出详细的提案，包括示例和高级 API。
阶段 2：提案包括正式规范，并考虑了边缘情况，例如私有字段与继承的交互。
阶段 3：浏览器供应商在他们的引擎中实现私有字段，提案经过实际使用和测试的完善。
阶段 4：提案通过所有测试并包含在 ECMAScript 标准中。