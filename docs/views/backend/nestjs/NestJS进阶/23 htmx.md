## 1.HTMLX
[htmx](https://htmx.docs-hub.com/) 让你可以直接在 `HTML` 中使用 `AJAX`、`CSS` 过渡、`WebSockets` 和 服务器推送事件，通过 属性 构建 现代用户界面，结合 简单性 和 超文本的强大功能。

htmx 体积小（[~14k min.gz'd](https://app.unpkg.com/htmx.org@2.0.7/files/dist)），无依赖，可扩展，并且与 `react` 相比，减少 了 67% 的代码量。
...
`HTMLX`特别适合那些希望保持页面轻量且可维护性高的项目。对于希望快速开发出交互式和动态网页的开发者来说，它是一个非常有用的工具。
...
+ **功能**: 这个触发器组合了多个事件和一个延迟器。`keyup` 在键盘按键抬起时触发，`changed` 在输入内容变化时触发，`delay:500ms` 意味着事件触发后会延迟`500`毫秒再发送请求。
...
```
## 19.htmx:load
+ [events事件与日志记录](https://htmx.docs-hub.com/docs/#events)
+ [events事件参考api](https://htmx.docs-hub.com/reference/#events)
...
+ `hx-trigger="keyup changed delay:500ms"`：在键盘按键抬起或输入变化时触发，且有`500`毫秒的延迟。
...
<div id="search-results"></div>
```
用户在输入框中键入内容时，系统会等待`500`毫秒，如果内容发生了变化（用户停止输入），将会发送 `GET` 请求到 `/trigger_delay`，并将结果显示在 `#search-results` 中。
